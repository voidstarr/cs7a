...
// rectangular: a:x, b:y
//
Point::Point(long double a, long double b, char sys) {
  if (sys == 'r') {
    x = a;
    y = b;
    setPolar();
...
void Point::setTheta(long double nTheta) {
    theta = nTheta;
    setRectangular();
}
...
long double Point::distFrom(const Point &p) {
...
Point Point::operator*(const long double m) {
